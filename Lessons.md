...
Этот механизм делает обход коллекций в Go удобным и читаемым!
...
nums2 = make([]int, len(nums))
```
Этот способ сначала создает нулевое значение для среза (nil), а затем выделяет память с помощью make. Однако это избыточно и редко используется.
...
---
### Примечание 
Указатели это довольно сложная тема для углубленного изучения, нужно быть очень аккуратным при их использовании, так что лучше отдельно изучить ее на других ресурсах по другим примерам и гайдам. Тут даны очень поверхностные знания о них!
---
...
---
### Примечание 
Интерфейсы это довольно сложная тема для углубленного изучения, так что лучше отдельно изучить ее на других ресурсах по другим примерам и гайдам. Тут даны очень поверхностные знания о них!
---
...
Смотри пример в `Exercise\14`, описание: 

### Функциональность:
Книга `Book`: структура с информацией о книге.
Читатель `User`: структура, содержащая имя и список взятых книг.
Библиотека `Library`: управляет книгами и пользователями.
Интерфейс `LibraryManager`: описывает основные операции библиотеки.
### Объяснение кода:
1. Структура `Book` содержит заголовок, автора и статус книги (занята или нет).
2. Структура `User` содержит имя и список книг, которые он взял.
3. Интерфейс `LibraryManager` определяет методы управления библиотекой.
4. Структура `Library` реализует интерфейс `LibraryManager`:
	* `AddBook()` – добавляет книги в библиотеку.
	* `BorrowBook()` – позволяет пользователю взять книгу.
	* `ReturnBook()` – позволяет пользователю вернуть книгу.
	* `ListBooks()` – отображает доступные книги.
---
